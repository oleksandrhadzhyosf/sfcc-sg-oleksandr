...
/**
 * Remove all item from cart and redirect to cart show endpoint
 */
function deleteItems() {
    const cart = app.getModel('Cart').get();
    
    if (!empty(cart)) {
        const Transaction = require('dw/system/Transaction');
        let allProductLineItems = cart.getAllProductLineItems();
        Transaction.begin();
            for (var i = 0; i < allProductLineItems.size(); i++) {
                cart.removeProductLineItem(allProductLineItems[i]);
            }
        Transaction.commit();
    }
...
/** Deelte all items from cart.
 * @see {@link module:controllers/Cart~deleteItems} */
 exports.DeleteItems = guard.ensure(['get','https'], deleteItems);
